  ...
  if (isLoading) {
    return (
      ...
      >
        ...
        <span className='text-grey-700'>Loading...</span>
      </div>
    );
  }
    ...
    <div className='flex w-90 p-10 items-center justify-center'>
      <ul className='md:grid gap-5 grid-cols-3 grid-rows-3 justify-center'>
        {data.map((product: Product) => (
          <li
            key={product.id}
            className='flex w-full flex-col justify-between items-center border p-2 mb-5 rounded-md shadow-md'
          >
            <Image
              src={product.image}
              alt={product.title}
            ...
            <div>
              ...
              <p className='text-xl font-bold text-slate-700'>Price: € {product.price.toFixed(2)}</p>
